...
# Executes the COPY
if [ -n "${FLAG_COPY}" ]; then
	cp -r "${origin_path}/clang-tidy" "${target_path}/.clang-tidy" || exit 1
	echo "> STATUS: Successfully copied .clang-tidy folder."
fi

# Executes the LINK
if [ -n "${FLAG_LINK}" ]; then
	ln -snf "${origin_path}/clang-tidy" "${target_path}/.clang-tidy" || exit 1
	echo "> STATUS: Successfully linked .clang-tidy folder."
fi
